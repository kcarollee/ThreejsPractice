...
- FASTER SPRITES -> (DO NOT CREATE NEW SPRITE MATERIAL FOR EACH SPRITE)
...
function main() {
	...
	const cameraZoomInPos = 0.2;
	const cameraZoomOutPos = 10;
	...
	scene.name = "scene1";
	//scene.background = new THREE.Color(0x5383ac)
	...
	for (let i = 0; i < 20; i++) {
		...
		let url = "./assets/student name cards/name (" + (i + 1) + ").png";
	...
	class DoorflapMesh {
		...
		animate() {
			// if (this.pointerIsInside && this.rotateDeg > -Math.PI){
			// 	this.frontpageMesh.rotation.y = this.rotateDeg;
			// 	this.rotateDeg -= 0.1;
			// }

			// else if (!this.pointerIsInside && this.rotateDeg < 0){
			// 	this.frontpageMesh.rotation.y = this.rotateDeg;
			// 	this.rotateDeg += 0.1;
			// }
	...
	for (let y = 0; y < rowNum; y++) {
		for (let x = 0; x < colNum; x++) {
			...
			let doorFlap = new DoorflapMesh(new THREE.Vector3(posX, posY, 0), doorflapSize - 0.01,
				studentNameCards[inc % 20], studentProfilePictures[inc % 2]);
	...
	function updateRaycaster2() {
		...
		scene2StartCounter++;
		console.log(scene2StartCounter)
		intersects = raycaster.intersectObjects(currentScene.children, true);
	...
	function animateCamera() {
		...
		if (triggerCameraZoomIn) {
			...
			triggerSpriteScaleDown = false;
			currentCamera.position.z += (cameraZoomOutPos - currentCamera.position.z) * 0.1;
			if (Math.abs(currentCamera.position.z - cameraZoomOutPos) < 0.01) {
				zoominMode = false;
				console.log("STOP ZOOM OUT");
				triggerCameraZoomOut = false;
				return;
			}
			currentCamera.position.y += (0 - currentCamera.position.y) * 0.1;
			currentCamera.position.x += (0 - currentCamera.position.x) * 0.1;
		}
	...
	function onScroll(event) {
		...
		if (!zoominMode) {
			...
			if (scrollTop > 2000) { // Adjust the value to show the menu earlier or later
				currentScene = scene2;
			...
			// if (getMenuBarOpacity() == 1){
			// 	currentScene = scene2;
			// 	currentCamera = camera2;
			// 	currentCamera.position.z = cameraZoomOutPos;
			// }
			// else {
			// 	currentScene = scene;
			// 	currentCamera = camera;
