...
void main(){
  ...
  float noiseAmp = 2.0; // 2~ 20
  float noisePeriod = 10.0; // 2 ~ 20
  float nOffset = 5.0;
  float noiseX = cnoise(vec3(uv * noisePeriod, time)) * noiseAmp;
  float noiseY = cnoise(vec3(uv * noisePeriod, time + nOffset)) * noiseAmp;
  vec2 noiseUV = pc(vec2(noiseX, noiseY));
  
  

  float dist = length(uv - mouse);
 
  dist = 1.0 / pow(dist, 1.0);
   vec2 uvMod = uv;
  vec3 bbCol = texture2D(backbuffer, noiseUV).rgb;
 
  vec3 texCol = texture2D(texture, uvMod).rgb;
  
  
  

  outCol += texCol + bbCol * 0.999;
