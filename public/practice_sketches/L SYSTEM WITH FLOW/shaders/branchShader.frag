...
void main(){
  ...
  float n1 = noise(vec3(uv.x * noiseDensity, uv.y * noiseDensity, time));
  float n2 = noise(vec3(uv.x * noiseDensity, uv.y * noiseDensity, time + 3.0));
  uv.x += n1 * displacementStrength;
  uv.y += n2 * displacementStrength;
  
  uv.y = 1.0 - uv.y;
  
  vec3 texCol = texture2D(texBranch, uv).rgb;
