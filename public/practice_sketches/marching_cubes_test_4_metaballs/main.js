    ...
    for (let i = 0; i < 16; i++) {
      ...
      // the functions below yields overlapping values for some of the vertices, resulting in distorted shapes.
        ...
        switch (funcIndex) {
          ...
          case 2:
            hn =
              noise.simplex2(hx * 0.001, hy * 0.001) +
              noise.simplex2(hy * 0.001, hz * 0.001);
            break;
          case 3:
            hn =
              noise.simplex2(hx * 0.001, hy * 0.001) *
...
class MarchingCubes {
      ...
      for (let d = dStart; d < dEnd; d += singleCubeParams.depth) {
        for (let w = wStart; w < wEnd; w += singleCubeParams.width) {
          let cube = new Cube(
            ...
            h,
            d,
            singleCubeParams.width,
            singleCubeParams.height,
            singleCubeParams.depth
          );
  ...
  updateCubes() {
    ...
    if (!this.addedToScene) {
      ...
      this.totalGeom.setAttribute(
        "position",
        new THREE.BufferAttribute(vertFloat, 3)
      );
...
function main() {
  ...
  class MetaBall {
    ...
    updatePos() {
      ...
      let randDeg1 =
        noise.simplex2(step * 0.0025 + this.randX, this.randY) * Math.PI * 2;
  ...
  let marchingCubes = new MarchingCubes(
    30.0,
    30.0,
    30.0,
    2.0,
